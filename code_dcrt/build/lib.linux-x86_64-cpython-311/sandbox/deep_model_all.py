...
class DNN(nn.Module):
    ...
    def __init__(self, in_size, nb_knockoffs, use_cnn=False):
        super().__init__()
        # TO REVIEW
        if use_cnn:
            self.layers = nn.Sequential(
                # CNN layer
                nn.Conv1d(nb_knockoffs, 1, 1),
...
def _evaluate_final(model, loader, device, prob_type):
    ...
    model.gradients = torch.cat([torch.autograd.grad(x['pred_val'], x['X'], torch.ones_like(
        x['pred_val']))[0] for x in outputs], dim=0)
    model.gradients = torch.abs(torch.transpose(torch.mean(model.gradients, dim=0), 0, 1))
    gradients_desc, _ = torch.sort(model.gradients, dim=1, descending=True)

    k_j = torch.argmax(model.gradients, dim=1)
    median_j, _ = torch.median(gradients_desc[:, 1:], dim=1)
    tho_j = gradients_desc[:, 0] - median_j

    median_w_j, _ = torch.median(model.gradients[:, 1:], dim=1)
    w_j = model.gradients[:, 0] - median_w_j
    max_w_j, _ = torch.max(model.gradients[:, 1:], dim=1)
    cond_max = model.gradients[:, 0] >= max_w_j
    model.w_j = (w_j * cond_max).cpu().detach().numpy()


def _train_dnn(train_loader, val_loader, original_loader, p=50, nb_knockoffs=5, n_epochs=250, lr=1e-3, beta1=0.9,
               beta2=0.999, eps=1e-8, l1_weight=1e-4, l2_weight=0, save_file=None, verbose=2,
               random_state=2021, prob_type='regression', use_cnn=False):
    ...
    accuracy.to(device)
    # DNN model
    model = DNN(p, nb_knockoffs, use_cnn=use_cnn)
    model.to(device)
    ...
    best_loss = 1e100
    best_model = DNN(p, nb_knockoffs, use_cnn=use_cnn)
    best_epoch = 1
    ...
    _evaluate_final(best_model, original_loader, device, prob_type)
    return best_model
...
def _ensemble_dnnet(X, y, n_ensemble=100, verbose=1, bootstrap=True, split_perc=0.8,
    ...
    for lay in range(m):
        ...
        X_valid_scaled[:, lay, :] = scalers_list[lay].transform(
            X_valid[:, lay, :])
        X_scaled[:, lay, :] = scalers_list[lay].transform(X[:, lay, :])
    ...
    original_loader = _Dataset_Loader(
        X_scaled, tensor_y, batch_size=batch_size_val)
    current_model = _train_dnn(train_loader, validate_loader, original_loader,
                               p=X_train.shape[2], nb_knockoffs=m, save_file=save_file,
                               verbose=verbose, random_state=random_state, prob_type=prob_type, use_cnn=use_cnn)
