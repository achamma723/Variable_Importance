...
def holden_knockoff_aggregation(
    X,
    y,
    n_bootstraps=12,
    fdr=0.1,
    ...
    threshold = _holden_knockoff_threshold(
        holden_statistics, n_bootstraps=n_bootstraps, fdr=fdr, offset=offset
    )
...
def _holden_knockoff_statistic(
    X,
    y,
    n_bootstraps=12,
    construct_method="equi",
    ...
    if isinstance(random_state, (int, np.int32, np.int64)):
        ...
        X_tildes = parallel(
            delayed(gaussian_knockoff_generation)(
                X, mu, Sigma, method=construct_method, seed=seed
            )
            for seed in seeds
        )
    ...
    X_kos = [np.column_stack([X, X_tildes[i]]) for i in range(n_bootstraps)]

    if loss == "least_square":
        clf = LassoCV(cv=5, max_iter=1e4)
        clfs = parallel(
            delayed(clone(clf).fit)(X_kos[i], y) for i in range(n_bootstraps)
        )
        coefs = [clfs[i].coef_ for i in range(n_bootstraps)]

    elif loss == "logistic":
        clf = LogisticRegressionCV(
            penalty="l1", max_iter=1e4, solver="liblinear", cv=5, tol=1e-6
        )
        clfs = parallel(
            delayed(clone(clf).fit)(X_kos[i], y) for i in range(n_bootstraps)
        )
    ...
    abs_knockoff_coefs = [np.abs(coefs[i][n_features:]) for i in range(1, middle_index)]
    ...
    W_ko = np.ravel(
        [abs_coef - compared_knockoff_coefs for abs_coef in abs_knockoff_coefs]
    )

    return W_true, W_ko


def _holden_knockoff_threshold(holden_statistics, n_bootstraps=12, fdr=0.1, offset=1):
...
def emery_knockoff_aggregation(
    X,
    y,
    fdr=0.1,
    offset=1,
    ...
    X_tilde = _knockoff_equi_simultaneous(
        X, mu, Sigma, n_bootstraps=n_bootstraps, seed=seed
    )
...
def _emery_knockoff_threshold(statistics, fdr=0.1, offset=1, c=0.5, lamb=0.5):
    ...
    original_ranks = np.array(
        [find_rank_original_score(statistics[i, :]) for i in range(n_features)]
    )
    Ls = np.array(
        [calculate_L(original_ranks[i], d1, ic, i_lambda) for i in range(n_features)]
    )
    ...
    if found:
        threshold = count
        selected = sorted_ascore_index[np.where(Ls_sorted[:threshold] == 1)[0]]
    else:
        threshold = None
        selected = np.array([])
