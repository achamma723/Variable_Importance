...
def cluster_dcrt(
    ...
    _, pvals_group, Ts_group = dcrt_zero_cached(
        X_reduced,
        y,
        estimated_coef=estimated_coef,
        max_iter=max_iter,
...
def cluster_dcrt_aggregation(
    ...
    pvals = np.array(
        parallel(
            delayed(broadcast_metric)(pval, label, n_features)
...
def cluster_knockoff_aggregation(
    X,
    y,
    n_clusters,
    linkage="ward",
    ...
    cov_estimator="ledoit_wolf",
    adaptive=False,
    cluster_adaptive=False,
    train_size=0.7,
    gamma=0.5,
    gamma_min=0.05,
    verbose=False,
    fdr_control="bhq",
    single_aggregation=True,
    n_jobs=1,
    joblib_verbose=0,
    memory=None,
    random_state=None,
    ...
    results = Parallel(n_jobs_ecko)(
        delayed(knockoff_aggregation)(
            X,
            y,
            offset=offset,
            n_jobs=n_jobs_ako,
    ...
    if single_aggregation and n_bootstraps > 1:
        ...
        test_scores = np.array(
            [
                broadcast_metric(results[i][1], cluster_labels[i], n_features)
                for i in range(n_rand)
            ]
        )
        coef_signs = [
...
def cluster_dlasso(
    ...
    X_reduced, labels = _cluster_data(
        X,
        n_clusters,
        train_size=train_size,
        connectivity=connectivity,
    ...
    _, zscore_group, pval_group = desparsified_lasso(
        X_reduced, y, tol=tol, centered=True, method=method, c=c
    )
...
def cluster_dlasso_aggregation(
    X,
    y,
    n_clusters,
    connectivity=None,
    ...
    temp = parallel(
        delayed(dl_cached)(X_reduced, y, tol=tol, method=method, c=c)
        for X_reduced in Xs_reduced
    )
