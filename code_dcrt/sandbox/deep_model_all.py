...
def _evaluate_final(model, loader, device, prob_type):
    ...
    outputs = [model._validation_step(batch, device, prob_type) for batch in loader]
    model.gradients = torch.cat(
        [
            torch.autograd.grad(x["pred_val"], x["X"], torch.ones_like(x["pred_val"]))[
                0
            ]
            for x in outputs
        ],
    ...
    model.gradients = torch.abs(
        torch.transpose(torch.mean(model.gradients, dim=0), 0, 1)
    )
...
def _ensemble_dnnet(
    ...
    original_loader = _Dataset_Loader(X_scaled, tensor_y, batch_size=batch_size_val)
