...
def cluster_group_lasso_cv(
    ...
    if use_sklearn_backend:
        clf = MultiTaskLassoCV(
            cv=cv, n_alphas=10, n_jobs=n_jobs, verbose=joblib_verbose, random_state=rand
        )
    ...
    if use_l1_reg:
        param_grid = dict(group_reg=np.logspace(-5, 5, 5), l1_reg=np.logspace(-5, 5, 5))
    else:
...
def clustered_group_lasso_inference(
    X,
    y,
    n_clusters,
    linkage="ward",
    ...
    X_tilde = gaussian_knockoff_generation(X, mu, Sigma, seed=seed, memory=memory)

    X_ko = np.column_stack([X, X_tilde])
    mem = check_memory(memory)

    gr_coef = mem.cache(cluster_group_lasso_cv, ignore=["n_jobs", "joblib_verbose"])(
        X_ko,
        y,
        n_clusters,
        linkage=linkage,
...
def ensemble_clustered_group_lasso(
    X,
    y,
    n_clusters,
    linkage="ward",
    ...
    temp = parallel(
        delayed(clustered_group_lasso_inference)(
            X,
            y,
            n_clusters,
            linkage=linkage,
    ...
    pvals = parallel(
        delayed(_empirical_pval)(test_score, offset=offset)
        for test_score in test_scores
    )

    aggregated_pvals = quantile_aggregation(
        np.array(pvals), gamma=gamma, adaptive=adaptive
    )
