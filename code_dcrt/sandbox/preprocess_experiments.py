...
def preprocess_hcp(
    data_dir="/data/parietal/store/data/HCP900/",
    n_subjects=150,
    ...
    if experiment == "REST":
        ...
        masker = MultiNiftiMasker(
            mask_img=mask_img, n_jobs=n_jobs, verbose=1, memory=memory
        )
        mask = mask_img.get_data().astype(bool)
...
def preprocess_haxby(subject=2, memory=None, comparision="face_house"):
    ...
    if comparision == "face_house":
        ...
        condition_mask = np.logical_or(conditions == b"cat", conditions == b"chair")
    ...
    masker = NiftiMasker(
        mask_img=masker.mask_img_, standardize=True, smoothing_fwhm=None, memory=memory
    )
