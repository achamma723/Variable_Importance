...
def compute_d0crt(x, y, loss = "least_square",
    ...
    return pd.DataFrame({'method': 'd0crt',
                         'variable':x.columns,
...
def compute_RF(X, y, n_tree=100, k_fold=0, n_jobs=100,
               random_state=2022):
    var_labl = X.columns
    ...
    for index_i, (train_index, test_index) in enumerate(kf.split(X)):
        ...
        list_score.append(r2_score(y.loc[test_index], rf.predict(X.iloc[test_index, :])))
    list_imp = np.mean(np.array(list_imp), axis=0)

    return pd.DataFrame({'method': 'RF',
                         'variable':var_labl,
                         'importance':list_imp,
                         'p_value':np.nan,
...
def compute_cpi_permfitDnn(x, y, conditional=False, n_jobs=10, k_fold=0,
                           random_state = 2022, list_nominal=None, groups=None,
                           group_stacking=False, n_perm=100):
    rf = RandomForestRegressor(random_state=2023)
    dict_hyper = {'max_depth':[2, 5, 10, 20]}
    bbi_model = BlockBasedImportance(importance_estimator='Mod_RF', prob_type="regression",
                                     conditional=conditional, k_fold=k_fold, n_jobs=n_jobs,
                                     list_nominal=list_nominal, group_stacking=group_stacking,
                                     groups=groups, random_state=random_state,
                                     n_perm=n_perm, verbose=10)
    bbi_model.fit(x, y)
    ...
    return pd.DataFrame({'method': [method] * len(var_labl),
                         'variable':var_labl,
...
def compute_marginal(X, y, groups={}, random_state=2022):
    ...
    return pd.DataFrame({'method': 'Marginal',
...
def loop_params(X, y, nominal_columns=[], grps={}, param_grid={}, n_jobs=1,
                k_fold=0, title_res="new_file.csv"):
    count_res = 0
    for params in list(ParameterGrid(param_grid)):
        ...
        if params['method'] == 'permfit-dnn':
            ...
            res = compute_cpi_permfitDnn(X, y, conditional=False, k_fold=k_fold,
                                        random_state=2022, n_jobs=n_jobs, list_nominal=nominal_columns,
                                        group_stacking=group_stacking, groups=grps, n_perm=50)
            res['time'] = (time.monotonic() - start_time) / 60
            res = res.sort_values(by=["p_value"], ascending=True)
        ...
        if params['method'] == 'cpi-dnn':
            ...
            res = compute_cpi_permfitDnn(X, y, conditional=True, k_fold=k_fold,
                                        random_state=2022, n_jobs=n_jobs, list_nominal=nominal_columns,
                                        group_stacking=group_stacking, groups=grps, n_perm=50)
            res['time'] = (time.monotonic() - start_time) / 60
            res = res.sort_values(by=["p_value"], ascending=True)
        ...
        if params['method'] == 'RF':
            ...
            res = compute_RF(X, y, n_jobs=n_jobs, k_fold=k_fold,
                             n_tree=2000, random_state=2022)
