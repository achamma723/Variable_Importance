...
def calculate_cvg(est, se, true_vi, level=0.05):
...
def lazy_train_cv(
    ...
    for lam in lam_path:
        for train, test in kf.split(X_train_change):
            ...
            lazy_pred_train, lazy_pred_test = lazy_predict(
                grads_train,
                flat_params,
                full_nn,
                hidden_layers,
                shape_info,
                X_train_change[train],
                y_train[train],
                X_train_change[test],
                lam,
            )
...
def vi_experiment_wrapper(
    X,
    y,
    network_width,
    ix=None,
    exp_iter=1,
    lambda_path=np.logspace(0, 2, 10),
    lam="cv",
    lazy_init="train",
    do_retrain=True,
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=0.33, random_state=exp_iter
    )
    trainset = torch.utils.data.TensorDataset(
    ...
    for j in ix:
        ...
        dr_vi = (
            nn.MSELoss()(dr_pred_test, y_test).item()
            - nn.MSELoss()(full_pred_test, y_test).item()
        )
        ...
        results.append(
            [varr, "lazy", lazy_time, lazy_vi, lazy_train_loss, lazy_test_loss, se]
        )
        ...
        if lazy_init == "random":
            ...
            for k, param in enumerate(random_nn.parameters()):
                param.data = lazy_retrain_Tlist[k] + np.random.normal(
                    size=lazy_retrain_Tlist[k].shape
                )
        ...
        if do_retrain:
            ...
            vi_retrain = (
                nn.MSELoss()(retrain_pred_test, y_test).item()
                - nn.MSELoss()(y_test, full_pred_test).item()
            )
        ...
        if early_stop:
            t0 = time.time()
            retrain_pred_train, retrain_pred_test = fake_retrain(
                p,
                full_nn,
                hidden_layers,
                j,
        ...
        if include_linear:
            ...
            vi_linear = mse(
                lmj.predict(X_test_change.detach().numpy()), y_test.detach().numpy()
            ) - mse(lm.predict(X_test.detach().numpy()), y_test.detach().numpy())
            loss_rt_test = mse(
                lmj.predict(X_test_change.detach().numpy()), y_test.detach().numpy()
            )
            loss_rt_train = mse(
                lmj.predict(X_train_change.detach().numpy()), y_train.detach().numpy()
            )
            results.append(
                [varr, "ols", lin_time, vi_linear, loss_rt_train, loss_rt_test]
            )
        ...
        if include_rf:
            ...
            lin_time = time.time() - t0
            vi_linear = mse(
                rfj.predict(X_test_change.detach().numpy()), y_test.detach().numpy()
            ) - mse(rf.predict(X_test.detach().numpy()), y_test.detach().numpy())
            loss_rt_test = mse(
                rfj.predict(X_test_change.detach().numpy()), y_test.detach().numpy()
            )
            loss_rt_train = mse(
                rfj.predict(X_train_change.detach().numpy()), y_train.detach().numpy()
            )
            results.append(
                [varr, "rf", lin_time, vi_linear, loss_rt_train, loss_rt_test]
            )

    df = pd.DataFrame(
        results,
        columns=["variable", "method", "time", "vi", "train_loss", "test_loss", "se"],
    )
    return df
...
def boxplot_2d(x, y, ax, co="g", whis=1.5, method=""):
    ...
    ##the x median
    vline = Line2D(
        [xlimits[1], xlimits[1]], [ylimits[0], ylimits[2]], color=co, zorder=1
    )
    ax.add_line(vline)

    ##the y median
    hline = Line2D(
        [xlimits[0], xlimits[2]], [ylimits[1], ylimits[1]], color=co, zorder=1
    )
    ax.add_line(hline)
    ...
    whisker_line = Line2D(
        [left, xlimits[0]], [ylimits[1], ylimits[1]], color=co, zorder=1
    )
    ax.add_line(whisker_line)
    whisker_bar = Line2D([left, left], [ylimits[0], ylimits[2]], color=co, zorder=1)
    ax.add_line(whisker_bar)

    ##right
    right = np.max(x[x < xlimits[2] + whis * iqr])
    whisker_line = Line2D(
        [right, xlimits[2]], [ylimits[1], ylimits[1]], color=co, zorder=1
    )
    ax.add_line(whisker_line)
    whisker_bar = Line2D([right, right], [ylimits[0], ylimits[2]], color=co, zorder=1)
    ax.add_line(whisker_bar)
    ...
    whisker_line = Line2D(
        [xlimits[1], xlimits[1]], [bottom, ylimits[0]], color=co, zorder=1
    )
    ax.add_line(whisker_line)
    whisker_bar = Line2D([xlimits[0], xlimits[2]], [bottom, bottom], color=co, zorder=1)
    ax.add_line(whisker_bar)

    ##top
    top = np.max(y[y < ylimits[2] + whis * iqr])
    whisker_line = Line2D(
        [xlimits[1], xlimits[1]], [top, ylimits[2]], color=co, zorder=1
    )
    ax.add_line(whisker_line)
    whisker_bar = Line2D([xlimits[0], xlimits[2]], [top, top], color=co, zorder=1)
